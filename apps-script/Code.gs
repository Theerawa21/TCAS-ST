...
const STUDENT_SPREADSHEET_ID = DATA_SPREADSHEET_ID;
...
const PORTFOLIO_APP_URL = 'https://theerawa21.github.io/TCAS-ST/';
...
function studentSheet_() {
  const sh = SpreadsheetApp.openById(DATA_SPREADSHEET_ID).getSheetByName(STUDENT_SHEET_NAME);
  if (!sh) throw new Error('ไม่พบชีต ' + STUDENT_SHEET_NAME);
  return sh;
}
...
function requiredStudentColumn_(headers, name) {
  const column = Number(headers[String(name).toLowerCase()] || 0);
  if (!column) throw new Error('ไม่พบคอลัมน์ ' + name + ' ในชีต ' + STUDENT_SHEET_NAME);
  return column;
}

function studentFromRow_(row, headers) {
  const read = name => {
    const column = Number(headers[String(name).toLowerCase()] || 0);
    return column ? String(row[column - 1] || '').trim() : '';
  };
  return {
    citizen_id:read('citizen_id'),
    student_id:read('student_id'),
    class_room:read('class_room'),
    title:read('title'),
...
function requestNote_(requestId) {
  return requestId ? 'request_id:' + requestId : '';
}

function completedRequestEntry_(sh, cfg, student, requestId) {
  if (!requestId || sh.getLastRow() < 2) return '';
  const entryIdColumn = recordEntryIdColumn_(sh, cfg, false);
  if (!entryIdColumn) return '';

  const count = sh.getLastRow() - 1;
  const notes = sh.getRange(2, entryIdColumn, count, 1).getNotes();
  const entryIds = sh.getRange(2, entryIdColumn, count, 1).getDisplayValues();
  const citizens = sh.getRange(2, 1, count, 1).getDisplayValues();
  const wanted = requestNote_(requestId);

  for (let i = 0; i < count; i++) {
    if (notes[i][0] === wanted && String(citizens[i][0]) === student.citizen_id) {
      return String(entryIds[i][0] || '');
    }
  }
  return '';
}

function markRequestCompleted_(sh, row, entryIdColumn, requestId) {
  if (requestId) sh.getRange(row, entryIdColumn).setNote(requestNote_(requestId));
}
...
function saveRecord_(p) {
  ...
  const cfg = CONFIG[type];
  const requestId = normalizedRequestId_(p.request_id);
  validate_(type, p);
  ...
  try {
    ...
    const completedEntryId = completedRequestEntry_(sh, cfg, student, requestId);
    if (completedEntryId) return {entry_id:completedEntryId, attachments:[], duplicate:true};
    ...
    markRequestCompleted_(sh, target, entryIdColumn, requestId);
    clearStudentRecordCache_(student);
    clearTeacherDashboardCache_();
    return {entry_id:entryId, attachments:attachments, duplicate:false};
  } finally {
    lock.releaseLock();
  }
}
...
function updateRecord_(p) {
  ...
  try {
    ...
    markRequestCompleted_(sh, row, entryIdColumn, requestId);
    clearStudentRecordCache_(student);
    clearTeacherDashboardCache_();
    return {entry_id:entryId, attachments:attachments, duplicate:false};
  } finally {
...
function getAttachmentMap_(citizenId, ss) {
...
function getReviewMap_(citizenId, ss) {
  ss = ss || SpreadsheetApp.openById(DATA_SPREADSHEET_ID);
...
function getAllActiveStudents_(ss) {
  const sh = ss ? ss.getSheetByName(STUDENT_SHEET_NAME) : studentSheet_();
  if (!sh) throw new Error('ไม่พบชีต ' + STUDENT_SHEET_NAME);
  const headers = studentHeaderMap_(sh);
